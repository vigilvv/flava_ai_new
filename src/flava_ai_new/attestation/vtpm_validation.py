...
class VtpmValidation:
    ...
    def _decode_and_validate_oidc(
        ...
        """
        res = self._get_well_known_file(
            self.expected_issuer, self.oidc_endpoint).json()
    ...
    def _decode_and_validate_pki(
        ...
        """
        res = self._get_well_known_file(
            self.expected_issuer, self.pki_endpoint).content
        ...
        calculated_fingerprint = ":".join(
            format(b, "02x") for b in fingerprint).upper()
    ...
    def _extract_and_validate_certificates(
        ...
        try:
            certs = [self._decode_der_certificate(
                cert) for cert in x5c_headers]
    ...
    def _compare_root_certificates(
        ...
        try:
            fingerprint1 = hashlib.sha256(
                root_cert.tbs_certificate_bytes).digest()
    ...
    def _verify_certificate_chain(certificates: PKICertificates) -> None:
        ...
        try:
            ...
            store.add_cert(X509.from_cryptography(
                certificates.intermediate_cert))
